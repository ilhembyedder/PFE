...
| **Move off Ant Design** to Tailwind + Radix/shadcn | Full token control, no v5/v6 drift, components owned in-repo rather than themed around. | Substantial. `Table` (server pagination, sorting, filtering), `Steps`, `Timeline`, `Upload` and `Form` validation must be rebuilt on TanStack Table and react-hook-form. `architecture.md` and `ux-design-specification.md` both name Ant Design and require updating. |
| **The UX specification is a starting point, not binding** | The new direction is derived from the product rather than from the existing UI. | `ux-design-specification.md` should be revised to match the new direction so the project documentation stays internally consistent. Its *principles*, component strategy and accessibility strategy are retained; its palette, theme and typography are not. |
