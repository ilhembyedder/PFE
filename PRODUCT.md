...
| **Move off Ant Design** to Tailwind v4 + shadcn on **Base UI** | Full token control, no v5/v6 drift, components owned in-repo rather than themed around. shadcn now ships on Base UI, the successor to Radix primitives from the same team. | Substantial, though less than first assessed: shadcn provides `Data Table`, `Sidebar`, `Breadcrumb`, `Empty`, `Field`, `Toast` and most other primitives. `Steps`, `Timeline` and the upload pipeline are still built from scratch. Our palette is mapped onto shadcn's semantic token names so components work unmodified. |
